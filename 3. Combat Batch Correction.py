...
if __name__ == "__main__":
    ...
    """
    df1 = pd.read_csv('Neg_Clean.csv')
    df1 = df1.set_index('chem_id')
    loc1 = df1.loc[:, 'Alignment ID':'MS/MS spectrum']
    loc2 = df1.loc[:, df1.columns.str.contains('P-Batch')]
    
    df2 = pd.read_csv('pheno-clean.csv')
    df2 = df2[df2['Sample Name'].isin(loc2.columns)]
    df2 = df2.set_index('Sample Name')
    #df3 = pd.read_csv('GestationalAge.csv')
    #df2 = pd.merge(df2, df3, on='Sample Name')
    #df2 = df2.set_index('Sample Name')
    ...
    pheno = df2
    dat = np.log10(loc2)
    mod = patsy.dmatrix("~ age", pheno, return_type="dataframe")
    import time
    t = time.time()
    ...
    ebat.to_csv("py-batch.csv")
    
    ebat = 10**ebat
    ebat = ebat.reset_index()
    loc1 = loc1.reset_index()
    ebat = pd.concat([loc1, ebat], axis=1)
    ebat.to_csv('Neg_CleanFnaBac.csv')
